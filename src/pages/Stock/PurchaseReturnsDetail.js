...
export class PurchaseReturnsDetail extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      selectedRowKeys: [],
        ...
        render: (text, record) => {
          ...
          <FormItem>
            {getFieldDecorator(`totalPrice`, {
              ...
              <span> { getFieldValue('price') * getFieldValue('qty') }</span>
            )}
          </FormItem>
          )
        }
        ...
        <Form style={{backgroundColor: '#fff', padding: '20px'}} onSubmit={this.handleSubmit}>
          ...
          <Card title="商品明细" className="mt-20">
            ...
            <Row gutter={24} style={{margin: 10}}>
              <Col sm={12}>
                总数：<span>{getFieldValue('qty') ? getFieldValue('qty') * dataSourceRows.length : 0}</span>
              </Col>
              <Col sm={12}>
                合计：<span>{getFieldValue('qty') ? getFieldValue('qty') * getFieldValue('price') * dataSourceRows.length : 0}</span>
              </Col>
            </Row>
            <Row gutter={24}>
              <Col sm={4}>
...
const PurchaseReturnsDetailForm = Form.create()(PurchaseReturnsDetail)
...
export default connect(mapStateToProps, mapDispatchToProps)(PurchaseReturnsDetailForm);
